"use strict";
let notasAlunos = [10, 8, 8.5, 7];
let notasAlunos2 = [5, 9, 7, 9.5];
...
let notaDoAluno = notasAlunos[0];
console.log(notaDoAluno);
let notasAlunosCopia = notasAlunos;
...
function checaAdulto(idade) {
    return idade >= 18;
}
let val = idades.every(checaAdulto);
...
var numbersMAP = numbers2.map(myFunction);
...
let novoTamanho = notasAlunos.push(10);
...
function getSubtracao(total, num) {
    return total - num;
}
let resultado = numbers3.reduceRight(getSubtracao);
...
let val2 = idades.some(checaAdulto);
...
let representacaoFrutas = fruits.toString();
